...
<?php
$year = 2024; // 輸入的年份
    $startYear = 1024; // 甲子年的年份
    $startIndex = 0; // 甲子年在天干地支中的索引
    ...
    // 計算天干和地支的索引
    $tianGanIndex = ($startIndex + $diff) % count($tianGan);
    $diZhiIndex = ($startIndex + $diff) % count($diZhi);

    // 輸出結果
    echo "西元{$year}年對應的天干地支為：{$tianGan[$tianGanIndex]}{$diZhi[$diZhiIndex]}<br>";
    echo "天干：甲乙丙丁戊己庚辛壬癸<br>";
    echo "地支：子丑寅卯辰巳午未申酉戌亥<br>";
?>
</body>
