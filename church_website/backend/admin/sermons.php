<?php
// D:\YU\church_website\backend\admin\sermons.php
...
// 如果是編輯模式，讀取單筆講道資料
if ($action === 'edit' && !empty($sermon_id)) {
    $stmt = $conn->prepare("SELECT * FROM sermons WHERE id = ?");
    $stmt->bind_param('i', $sermon_id);
    $stmt->execute();
    $result = $stmt->get_result();
    if ($result->num_rows === 1) {
        $sermon_to_edit = $result->fetch_assoc();
    }
    $stmt->close();
}
...
<head>
    ...
    <title>講道管理 - 教會網站管理系統</title>
    <link rel="stylesheet" href="../assets/css/admin_style.css"> <!-- 假設有一個共用的後台 CSS -->
    <style>
        /* 沿用儀表板樣式，並加上此頁專用樣式 */
        ...
        input[type="text"], input[type="date"], textarea { width: 100%; padding: .5rem; border: 1px solid #ccc; border-radius: 4px; box-sizing: border-box; }
...
<body>
    ...
    <div class="sidebar">
        ...
        <ul>
            ...
            <li><a href="sermons.php" class="active">講道管理</a></li>
    ...
    <main class="main-content">
        ...
        <div class="form-container">
            <h2><?php echo $sermon_to_edit ? '編輯講道' : '新增講道'; ?></h2>
            <form action="sermons.php" method="POST">
                <input type="hidden" name="id" value="<?php echo $sermon_to_edit['id'] ?? ''; ?>">
                <div class="form-group">
                    <label for="title">標題</label>
                    <input type="text" id="title" name="title" value="<?php echo htmlspecialchars($sermon_to_edit['title'] ?? ''); ?>" required>
                </div>
                <div class="form-group">
                    <label for="date">日期</label>
                    <input type="date" id="date" name="date" value="<?php echo htmlspecialchars($sermon_to_edit['date'] ?? ''); ?>" required>
                </div>
                <div class="form-group">
                    <label for="speaker">講員</label>
                    <input type="text" id="speaker" name="speaker" value="<?php echo htmlspecialchars($sermon_to_edit['speaker'] ?? ''); ?>" required>
                </div>
                <div class="form-group">
                    <label for="content">內容摘要</label>
                    <textarea id="content" name="content"><?php echo htmlspecialchars($sermon_to_edit['content'] ?? ''); ?></textarea>
                </div>
                <div class="form-group">
                    <label for="audio_url">音檔 URL</label>
                    <input type="text" id="audio_url" name="audio_url" value="<?php echo htmlspecialchars($sermon_to_edit['audio_url'] ?? ''); ?>">
                </div>
                <div class="form-group">
                    <label for="video_url">影片 URL</label>
                    <input type="text" id="video_url" name="video_url" value="<?php echo htmlspecialchars($sermon_to_edit['video_url'] ?? ''); ?>">
                </div>
                <div class="form-actions">
                    <button type="submit" class="btn-primary"><?php echo $sermon_to_edit ? '更新' : '新增'; ?></button>
                    <?php if ($sermon_to_edit): ?>
                        <a href="sermons.php" class="btn-secondary" style="text-decoration:none;">取消編輯</a>
                    <?php endif; ?>
                </div>
            </form>
        </div>
            ...
            <table>
                ...
                <tbody>
                    <?php if (empty($sermons)): ?>
                        <tr><td colspan="4">目前沒有任何講道紀錄。</td></tr>
                    <?php else: ?>
                        ...
                        <tr>
                            ...
                            <td class="actions">
                                <a href="sermons.php?action=edit&id=<?php echo $sermon['id']; ?>" class="edit">編輯</a>
                                <a href="sermons.php?action=delete&id=<?php echo $sermon['id']; ?>" class="delete" onclick="return confirm('您確定要刪除這筆紀錄嗎？');">刪除</a>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
