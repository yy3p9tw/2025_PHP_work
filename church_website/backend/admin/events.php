<?php
// D:\YU\church_website\backend\admin\events.php
...
// 處理刪除操作
if ($action === 'delete' && !empty($event_id)) {
    $stmt = $conn->prepare("DELETE FROM events WHERE id = ?");
    $stmt->bind_param('i', $event_id);
    if ($stmt->execute()) {
        $success_message = '活動紀錄刪除成功！';
    } else {
...
<body>
    ...
    <div class="sidebar">
        ...
        <ul>
            ...
            <li><a href="events.php" class="active">活動管理</a></li>
    ...
    <main class="main-content">
        ...
        <div class="form-container">
            <h2><?php echo $event_to_edit ? '編輯活動' : '新增活動'; ?></h2>
            <form action="events.php" method="POST">
                <input type="hidden" name="id" value="<?php echo $event_to_edit['id'] ?? ''; ?>">
                <div class="form-group">
                    <label for="title">標題</label>
                    <input type="text" id="title" name="title" value="<?php echo htmlspecialchars($event_to_edit['title'] ?? ''); ?>" required>
                </div>
                <div class="form-group">
                    <label for="date">日期</label>
                    <input type="date" id="date" name="date" value="<?php echo htmlspecialchars($event_to_edit['date'] ?? ''); ?>" required>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <textarea id="description" name="description"><?php echo htmlspecialchars($event_to_edit['description'] ?? ''); ?></textarea>
                </div>
                <div class="form-actions">
                    <button type="submit" class="btn-primary"><?php echo $event_to_edit ? '更新' : '新增'; ?></button>
                    <?php if ($event_to_edit): ?>
                        <a href="events.php" class="btn-secondary" style="text-decoration:none;">取消編輯</a>
                    <?php endif; ?>
                </div>
            </form>
        </div>
        ...
        <div class="list-container">
            ...
            <table>
                ...
                <tbody>
                    <?php if (empty($events)): ?>
                        <tr><td colspan="4">目前沒有任何活動紀錄。</td></tr>
                    <?php else: ?>
                        ...
                        <tr>
                            ...
                            <td class="actions">
                                <a href="events.php?action=edit&id=<?php echo $event['id']; ?>" class="edit">編輯</a>
                                <a href="events.php?action=delete&id=<?php echo $event['id']; ?>" class="delete" onclick="return confirm('您確定要刪除這筆紀錄嗎？');">刪除</a>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
